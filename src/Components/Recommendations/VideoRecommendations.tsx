...
import { VideoRecommendationConfigTranslations } from "../../config/InternationalizationConfig";
...
export const RecommendationListRenderer: FunctionComponent<RecommendationListProps> = (props) => {
  ...
  const type = VideoRecommendationConfig.type; // Assuming you have this type defined in the config
  const numberOfResults = VideoRecommendationConfig.numberOfResults;

  return (
    <MainWrapper>
      <Title>{VideoRecommendationConfig.title && getText(VideoRecommendationConfig.title, VideoRecommendationConfigTranslations, "title")}</Title>
      <SubTitle>{VideoRecommendationConfig.description && getText(VideoRecommendationConfig.description, VideoRecommendationConfigTranslations, "description")}</SubTitle>
      {type === 'carousel' ? (
        <>
          <SliderContainer>
        ...
        <NormalCardWrapper>
          {state.recommendations.length > 0 ? (
            state.recommendations.slice(0, numberOfResults).map((recommendation) => {
              const imageURL: string = recommendation.raw[`${VideoRecommendationConfig.imageField}`] as string;
              return (
